...
- **EAビルダー**: エントリー条件(MAクロス/RSI/BB/MACD/ドンチャン/ストキャス/一目/ケルトナー/CCI)と決済ルール(SL/TP/トレーリング/反対シグナル)をフォームで組み、手元のヒストリカルデータでバックテスト(勝率/PF/最大DD/資産曲線)。戦略から**コンパイル可能なMQL5/MQL4のEAソース**を生成・ダウンロードできます。
...
npm run diagnose:dukascopy -- --pair=AUDJPY --tf=d1,h1  # ペア・時間足を指定して診断
```
