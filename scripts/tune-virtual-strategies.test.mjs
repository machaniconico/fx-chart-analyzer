...
import { SAR_MIN_STEP } from '../src/lib/indicators';
...
const legacyEntryTypeExpectations = [
  ...
  {
    entryType: 'stochCross',
    label: 'ストキャス%K/%Dクロス順張り',
    entryCondition: { type: 'stochCross', kPeriod: 14, dPeriod: 3, smoothing: 3 },
    exit: { stopLossPips: 40, takeProfitPips: 80, closeOnOppositeSignal: true },
    timeframes: ['m30', 'h1'],
    parameterRanges: {
      stopLossPips: { min: 20, max: 110, step: 15 },
...
describe('tune-virtual-strategies candidate matrix and CLI filters', () => {
  it('keeps the required pair and entry-type coverage explicit', () => {
    ...
    expect(ENTRY_TYPE_PROFILES.cciBreak).toEqual({
      label: 'CCIブレイク順張り',
      timeframes: ['m30', 'h1'],
      entryCondition: { type: 'cciBreak', period: 14, level: 100 },
      exit: { stopLossPips: 25, takeProfitPips: 50, closeOnOppositeSignal: false },
    ...
    expect(ENTRY_TYPE_PROFILES.parabolicSar).toEqual({
      label: 'パラボリックSARフリップ順張り',
      timeframes: ['h1', 'h4'],
      entryCondition: { type: 'parabolicSar', step: SAR_MIN_STEP, maximum: 0.2 },
      exit: { stopLossPips: 40, takeProfitPips: 80, closeOnOppositeSignal: true },
      parameterRanges: {
        stopLossPips: { min: 20, max: 110, step: 15 },
        takeProfitPips: { min: 40, max: 220, step: 20 },
    ...
    expect(ENTRY_TYPE_PROFILES.momentum).toEqual({
      label: 'モメンタム100クロス順張り',
      timeframes: ['h1', 'h4'],
      entryCondition: { type: 'momentum', period: 14 },
      exit: { stopLossPips: 40, takeProfitPips: 80, closeOnOppositeSignal: true },
    ...
    expect(ENTRY_TYPE_PROFILES.rvi).toEqual({
      label: 'RVIシグナルラインクロス順張り',
      timeframes: ['h1', 'h4'],
      entryCondition: { type: 'rvi', period: 10 },
      exit: { stopLossPips: 40, takeProfitPips: 80, closeOnOppositeSignal: true },
    ...
    expect(ENTRY_TYPE_PROFILES.envelope).toEqual({
      label: 'エンベロープブレイク順張り',
      timeframes: ['h1', 'h4'],
      entryCondition: { type: 'envelope', period: 14, deviation: 0.1 },
      exit: { stopLossPips: 40, takeProfitPips: 80, closeOnOppositeSignal: true },
    ...
    expect(ENTRY_TYPE_PROFILES.stochCross).toEqual({
      label: 'ストキャス%K/%Dクロス順張り',
      timeframes: ['m30', 'h1'],
      entryCondition: { type: 'stochCross', kPeriod: 14, dPeriod: 3, smoothing: 3 },
      exit: { stopLossPips: 40, takeProfitPips: 80, closeOnOppositeSignal: true },
    ...
    expect(ENTRY_TYPE_PROFILES.alligator).toEqual({
      label: 'Alligator整列クロス順張り',
      timeframes: ['h1', 'h4'],
      entryCondition: {
        type: 'alligator',
        jawPeriod: 13,
  ...
  it('keeps the parabolicSar profile step within the SAR_MIN_STEP policy', () => {
    expect(ENTRY_TYPE_PROFILES.parabolicSar.entryCondition.step).toBeGreaterThanOrEqual(
      SAR_MIN_STEP,
    );
  });
  ...
  it('pins the Envelope SL/TP grids and 12-candidate magic-number block', async () => {
    ...
    expect(envelopeCandidates.map((target) => target.strategy.magicNumber)).toEqual([
      1783200050, 1783200051,
      1783200150, 1783200151,
      1783200250, 1783200251,
      1783200350, 1783200351,
      1783200450, 1783200451,
      1783200550, 1783200551,
    ]);
  });

  it('pins the stochCross 140-combination SL/TP grid', async () => {
    const { valuesFromRange } = await import('../src/lib/optimize.ts');
    const { stopLossPips, takeProfitPips } = ENTRY_TYPE_PROFILES.stochCross.parameterRanges;
    expect(valuesFromRange(stopLossPips)).toEqual([20, 35, 50, 65, 80, 95, 110]);
    expect(valuesFromRange(takeProfitPips)).toEqual([
      40, 60, 80, 100, 120, 140, 160, 180, 200, 220,
    ]);
    expect(
      valuesFromRange(stopLossPips).length *
        valuesFromRange(takeProfitPips).length *
        ENTRY_TYPE_PROFILES.stochCross.trailingStopPips.length,
    ).toBe(140);
  });

  it('pins the Alligator 140-combination SL/TP grid', async () => {
    const { valuesFromRange } = await import('../src/lib/optimize.ts');
    const { stopLossPips, takeProfitPips } = ENTRY_TYPE_PROFILES.alligator.parameterRanges;
    expect(valuesFromRange(stopLossPips)).toEqual([20, 35, 50, 65, 80, 95, 110]);
    expect(valuesFromRange(takeProfitPips)).toEqual([
      40, 60, 80, 100, 120, 140, 160, 180, 200, 220,
    ]);
    expect(
      valuesFromRange(stopLossPips).length *
        valuesFromRange(takeProfitPips).length *
        ENTRY_TYPE_PROFILES.alligator.trailingStopPips.length,
    ).toBe(140);
  });
  ...
  it('covers every pair, entry type, and suitable timeframe exactly once', () => {
    ...
    expect(matrix.filter((target) => target.entryType === 'alligator')).toHaveLength(12);
  ...
  it('assigns CCI candidates to entry-type index 8 with 12 candidates and unique magic numbers', () => {
    const matrix = buildCandidateMatrix();
    const cciCandidates = matrix.filter((target) => target.entryType === 'cciBreak');

    expect(cciCandidates).toHaveLength(12);
    expect(cciCandidates.map((target) => target.strategy.magicNumber)).toEqual([
      1783100080,
      1783100081,
      1783100180,
      1783100181,
  ...
  it('assigns Parabolic SAR candidates to the first second-block index with exact triples', () => {
    const matrix = buildCandidateMatrix();
    const parabolicSarCandidates = matrix.filter((target) => target.entryType === 'parabolicSar');

    expect(parabolicSarCandidates).toHaveLength(12);
    expect(
      parabolicSarCandidates.map((target) => [
        target.strategy.meta.pair,
        target.strategy.meta.timeframe,
        target.strategy.magicNumber,
      ]),
    ).toEqual([
      ['USDJPY', 'h1', 1783200000],
      ['USDJPY', 'h4', 1783200001],
      ['EURUSD', 'h1', 1783200100],
      ['EURUSD', 'h4', 1783200101],
      ['GBPJPY', 'h1', 1783200200],
      ['GBPJPY', 'h4', 1783200201],
      ['EURJPY', 'h1', 1783200300],
      ['EURJPY', 'h4', 1783200301],
      ['GBPUSD', 'h1', 1783200400],
      ['GBPUSD', 'h4', 1783200401],
      ['AUDJPY', 'h1', 1783200500],
      ['AUDJPY', 'h4', 1783200501],
    ]);
    expect(new Set(matrix.map((target) => target.strategy.magicNumber)).size).toBe(matrix.length);
  });

  it('assigns momentum candidates to the second second-block index with exact triples', () => {
    const matrix = buildCandidateMatrix();
    const momentumCandidates = matrix.filter((target) => target.entryType === 'momentum');

    expect(momentumCandidates).toHaveLength(12);
    expect(
      momentumCandidates.map((target) => [
        target.strategy.meta.pair,
        target.strategy.meta.timeframe,
        target.strategy.magicNumber,
      ]),
    ).toEqual([
      ['USDJPY', 'h1', 1783200010],
      ['USDJPY', 'h4', 1783200011],
      ['EURUSD', 'h1', 1783200110],
      ['EURUSD', 'h4', 1783200111],
      ['GBPJPY', 'h1', 1783200210],
      ['GBPJPY', 'h4', 1783200211],
      ['EURJPY', 'h1', 1783200310],
      ['EURJPY', 'h4', 1783200311],
      ['GBPUSD', 'h1', 1783200410],
      ['GBPUSD', 'h4', 1783200411],
      ['AUDJPY', 'h1', 1783200510],
      ['AUDJPY', 'h4', 1783200511],
    ]);
    expect(new Set(matrix.map((target) => target.strategy.magicNumber)).size).toBe(matrix.length);
  });

  it('assigns RVI candidates to the third second-block index with exact triples', () => {
    const matrix = buildCandidateMatrix();
    const rviCandidates = matrix.filter((target) => target.entryType === 'rvi');

    expect(rviCandidates).toHaveLength(12);
    expect(
      rviCandidates.map((target) => [
        target.strategy.meta.pair,
        target.strategy.meta.timeframe,
        target.strategy.magicNumber,
      ]),
    ).toEqual([
      ['USDJPY', 'h1', 1783200020],
      ['USDJPY', 'h4', 1783200021],
      ['EURUSD', 'h1', 1783200120],
      ['EURUSD', 'h4', 1783200121],
      ['GBPJPY', 'h1', 1783200220],
      ['GBPJPY', 'h4', 1783200221],
      ['EURJPY', 'h1', 1783200320],
      ['EURJPY', 'h4', 1783200321],
      ['GBPUSD', 'h1', 1783200420],
      ['GBPUSD', 'h4', 1783200421],
      ['AUDJPY', 'h1', 1783200520],
      ['AUDJPY', 'h4', 1783200521],
    ]);
    expect(new Set(matrix.map((target) => target.strategy.magicNumber)).size).toBe(matrix.length);
  });
  ...
  it('assigns Envelope candidates to entry-type index 15 with exact triples', () => {
    const matrix = buildCandidateMatrix();
    const envelopeCandidates = matrix.filter((target) => target.entryType === 'envelope');

    expect(envelopeCandidates).toHaveLength(12);
    expect(candidateMagicNumber(0, 15, 0)).toBe(1783200050);
    expect(
      envelopeCandidates.map((target) => [
        target.strategy.meta.pair,
        target.strategy.meta.timeframe,
        target.strategy.magicNumber,
      ]),
    ).toEqual([
      ['USDJPY', 'h1', 1783200050],
      ['USDJPY', 'h4', 1783200051],
      ['EURUSD', 'h1', 1783200150],
      ['EURUSD', 'h4', 1783200151],
      ['GBPJPY', 'h1', 1783200250],
      ['GBPJPY', 'h4', 1783200251],
      ['EURJPY', 'h1', 1783200350],
      ['EURJPY', 'h4', 1783200351],
      ['GBPUSD', 'h1', 1783200450],
      ['GBPUSD', 'h4', 1783200451],
      ['AUDJPY', 'h1', 1783200550],
      ['AUDJPY', 'h4', 1783200551],
    ]);
  });

  it('assigns stochCross candidates to entry-type index 16 with exact triples', () => {
    const matrix = buildCandidateMatrix();
    const stochCrossCandidates = matrix.filter((target) => target.entryType === 'stochCross');

    expect(stochCrossCandidates).toHaveLength(12);
    expect(candidateMagicNumber(0, 16, 0)).toBe(1783200060);
    expect(
      stochCrossCandidates.map((target) => [
        target.strategy.meta.pair,
        target.strategy.meta.timeframe,
        target.strategy.magicNumber,
      ]),
    ).toEqual([
      ['USDJPY', 'm30', 1783200060],
      ['USDJPY', 'h1', 1783200061],
      ['EURUSD', 'm30', 1783200160],
      ['EURUSD', 'h1', 1783200161],
      ['GBPJPY', 'm30', 1783200260],
      ['GBPJPY', 'h1', 1783200261],
      ['EURJPY', 'm30', 1783200360],
      ['EURJPY', 'h1', 1783200361],
      ['GBPUSD', 'm30', 1783200460],
      ['GBPUSD', 'h1', 1783200461],
      ['AUDJPY', 'm30', 1783200560],
      ['AUDJPY', 'h1', 1783200561],
    ]);
  });

  it('assigns Alligator candidates to entry-type index 17 with exact triples', () => {
    const matrix = buildCandidateMatrix();
    const alligatorCandidates = matrix.filter((target) => target.entryType === 'alligator');

    expect(alligatorCandidates).toHaveLength(12);
    expect(candidateMagicNumber(0, 17, 0)).toBe(1783200070);
    expect(
      alligatorCandidates.map((target) => [
        target.strategy.meta.pair,
        target.strategy.meta.timeframe,
        target.strategy.magicNumber,
      ]),
    ).toEqual([
      ['USDJPY', 'h1', 1783200070],
      ['USDJPY', 'h4', 1783200071],
      ['EURUSD', 'h1', 1783200170],
      ['EURUSD', 'h4', 1783200171],
      ['GBPJPY', 'h1', 1783200270],
      ['GBPJPY', 'h4', 1783200271],
      ['EURJPY', 'h1', 1783200370],
      ['EURJPY', 'h4', 1783200371],
      ['GBPUSD', 'h1', 1783200470],
      ['GBPUSD', 'h4', 1783200471],
      ['AUDJPY', 'h1', 1783200570],
      ['AUDJPY', 'h4', 1783200571],
    ]);
  });
  ...
  it('supports each filter independently', () => {
    ...
    expect(filterTargets(matrix, parseCliArgs(['--entry-type', 'stochCross']))).toHaveLength(12);
    expect(filterTargets(matrix, parseCliArgs(['--entry-type', 'alligator']))).toHaveLength(12);
  ...
  it('documents and filters the new entry types through the CLI', () => {
    ...
    expect(CLI_USAGE).toContain('stochCross');
    ...
    expect(CLI_USAGE).toContain('envelope');
    expect(
    ...
    expect(
      filterTargets(matrix, parseCliArgs(['--entry-type', 'STOCHCROSS'])),
    ).toEqual(
      matrix.filter((target) => target.entryType === 'stochCross'),
    );
    expect(
    ...
    expect(
      filterTargets(matrix, parseCliArgs(['--entry-type', 'PARABOLICSAR'])),
    ).toEqual(
      matrix.filter((target) => target.entryType === 'parabolicSar'),
    );
    expect(
      filterTargets(matrix, parseCliArgs(['--entry-type', 'MOMENTUM'])),
    ).toEqual(
      matrix.filter((target) => target.entryType === 'momentum'),
    );
    expect(
    ...
    expect(
      filterTargets(matrix, parseCliArgs(['--entry-type', 'ENVELOPE'])),
    ).toEqual(
      matrix.filter((target) => target.entryType === 'envelope'),
    );
  });
